...
emptyGrid (BoardInfo h w) = array ((1, 1), (w, h)) [((x, y), Empty) | y <- [1 .. h], x <- [1 .. w]]
...
buildInitialBoard ::
  ...
  Point ->
  RenderState
buildInitialBoard bf s a = RenderState added False
  where
    newBoard = emptyGrid bf
    added = newBoard // [(s, SnakeHead), (a, Apple)]
...
updateRenderState rs GameOver = RenderState (board rs) True
updateRenderState rs (RenderBoard rm) = RenderState (board rs // rm) False
...
render bf (RenderState b True) = foldr (\x acc -> ppCell x ++ acc) "" (emptyGrid bf)
render bf rs = undefined
